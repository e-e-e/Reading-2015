...
$(document).ready(function() {
	...
	var options = {
		...
		cookie_life : 60*60*3, //three hours
		cookie_domain : '.'+site,
		exclude: ['deappel.nl/webshop/'],
		stopable: true,
		time_constrained: false,
		click_tolerance: 3,
		dismiss_time: 60*30,// 30 minutes
		highlight_style: "color:grey",
		phrase_style: "text-decoration:underline;color:grey",
		...
		title_text: 'Benjamin Forster <span style="text-decoration:underline;">Reading ('+site+')</span> 2015<br><sub style="font-size:40%">click to dismiss » three times to stop completely</sub>',
		escape_element: '<div style="position:absolute;top:10px;right:10px;width:20px;height:10px;margin:0"><a style="font-family:\'avantgarde\', \'Helvetica Neue\', Helvetica, Arial, sans-serif;color:#000;text-decoration:none;font-size:20px;color:#000" href="javascript:">X</a></div>',
		text_element: '<h2 class="h0" style="font-family:\'avantgarde\', \'Helvetica Neue\', Helvetica, Arial, sans-serif; position:relative;top:50%;word-wrap:break-word;word-break:normal;"></h2>',
	};	
	...
	if(!skip && options.time_constrained) skip = within_date(4,20,22); // return false if 4 = thursday, 20 >= 8pm and 22 <= 10pm
	...
	function within_date(day,min,max) {
		var now = new Date();
		return !(now.getDay() === day && now.getHours() >= min && now.getHours() < max); //change this to 4.
	}
	...
	// FUNCTION DEFINITIONS
  	...
  	function process(first) {
  		...
  		if( delimiter === ';') {
	  		...
	  		//set state to display next phrase
  	...
  	function user_clicked() {
	  	if (clickable && options.stopable) {
		  	...
		  	docCookies.setItem('bf_clicks', user_data.c, options.dismiss_time,'/',options.cookie_domain);
